...
import { Package, AlertCircle, ShoppingCart, TrendingUp, Users, ShieldAlert, Cpu, IndianRupee, ArrowUpRight, ArrowDownRight, Clock, Activity, Zap } from 'lucide-react';
    ...
    <div className={cn("space-y-6 transition-opacity duration-300", isSyncing && "opacity-50 pointer-events-none")}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          <div 
            key={i} 
            onClick={() => setActiveTab?.(stat.label === 'Asset Value' ? 'inventory' : stat.label === 'Active Assets' ? 'warranty' : stat.label === 'Open Opportunities' ? 'crm' : 'complaints')}
